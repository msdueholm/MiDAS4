---
title: "Figure 3: MiDAS4 evaluation based on GWMC data"
...
a <- read_amp_mappings("data/GWMCASV_vs_MiDAS_2.1.3.b6")
b <- read_amp_mappings("data/GWMCASV_vs_MiDAS_3.7.b6")
c <- read_amp_mappings("data/GWMCASV_vs_MiDAS_4.8.b6")
d <- read_amp_mappings("data/GWMCASV_vs_GreenGenes_16S_13.5.b6")
e <- read_amp_mappings("data/GWMCASV_vs_RDP_16S_v16.b6")
f <- read_amp_mappings("data/GWMCASV_vs_SILVA_138_SSURef_NR99.b6")
g <- read_amp_mappings("data/GWMCASV_vs_GTDB_SSU_r89.b6")
...
# Read ASV table
ASVtable <- data.table::fread("data/GWMCASV.tab",
                      sep = "\t",
                      header = TRUE,
                      quote = "\"",
                      fill = TRUE,
...
df_long <- melt.data.table(df, id.vars = 1:1166, measure.vars = 1167:1173, variable.name = "Database", value.name = "Identity") 
df_long <- melt.data.table(df_long, id.vars = c("ASV","Database","Identity"), measure.vars = 2:1166, variable.name = "SeqID", value.name = "Counts") 
...
write.csv2(Summary1, file="output/Hits_summary_GWMC.txt", row.names=FALSE, quote=FALSE)
...
a2 <- read_amp_classifications("data/GWMCASV_vs_MiDAS_2.1.3.sintax")
b2 <- read_amp_classifications("data/GWMCASV_vs_MiDAS_3.7.sintax")
c2 <- read_amp_classifications("data/GWMCASV_vs_MiDAS_4.8.sintax")
d2 <- read_amp_classifications("data/GWMCASV_vs_GreenGenes_16S_13.5.sintax")
e2 <- read_amp_classifications("data/GWMCASV_vs_RDP_16S_v16.sintax")
f2 <- read_amp_classifications("data/GWMCASV_vs_SILVA_138_SSURef_NR99.sintax")
g2 <- read_amp_classifications("data/GWMCASV_vs_GTDB_SSU_r89.sintax")
...
# Rearrange data.table into long format with Database and SeqID as variables
df_long2 <- melt.data.table(df2, id.vars = c(1,1167:1168), measure.vars = 2:1166, variable.name = "SeqID", value.name = "Counts") 
...
write.csv2(Summary2, file="output/Genus_classification_summary_GWMC.txt", row.names=FALSE, quote=FALSE)
...
# Rearrange data.table into long format with Database and SeqID as variables
df_long3 <- melt.data.table(df3, id.vars = c(1,1167:1168), measure.vars = 2:1166, variable.name = "SeqID", value.name = "Counts") 
...
r0001c2 <- df_long3[Rel_abun>0.001 & Counts>0, .("Classified"=(.N-sum(is.na(S)))/.N*100,
            "detected_amplicons"=.N,
            "Cutoff"="0001",
            "Relative_abundance"=paste("Relative abundance > 0.001%\n Coverage: ",round(coverage_0001$mean[1],digits = 2),"% ±p3 ",round(coverage_0001$sd[1],digits = 2),"%, n=", coverage_0001$n[1],sep="")), by=.(Database,SeqID)]
...
write.csv2(Summary3, file="output/Species_classification_summary_GWMC.txt", row.names=FALSE, quote=FALSE)
...
## Export data
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
ggsave(filename="output/Figure3_Database_evaluation_GWMC.pdf", plot=p, width=9, height=12, useDingbats=FALSE, limitsize=FALSE)
```