---
title: "Figure 5 and S5: Beta-diversity for V1-V3"
...
V13metadata <- V13metadata[,-1] %>%
  mutate(V13_seq_id=gsub("-",".",V13_seq_id))


#Load V13ASV table and taxonomy manually
V13ASVtab <- read.csv("data/V13_ASVtab.txt", sep="\t") %>%
  rename("ASV" = X.OTU.ID)

V13ASVtax <- read.csv("data/V13ASV_vs_MiDAS_4.8.sintax", sep="\t", header = FALSE) %>%
  select(1,4) %>%
  mutate(V4=gsub(":","__",V4)) %>%
  separate(V4, c("Kingdom","Phylum","Class","Order","Family","Genus","Species"), sep=",") %>%
  rename("ASV" = V1)

usearch_otutable <- merge.data.frame(V13ASVtab, V13ASVtax, by ="ASV", all.x = TRUE)
...
d13nAS4PT_genus <- amp_subset_taxa(d13nAS4PT, tax_vector = ASVs_nogenus, remove = TRUE)
```
##Create ordinations
```{r}
# Remove samples with missing metadata
d13nAS4PT_genus <- amp_subset_samples(d13nAS4PT_genus, !(is.na(Temp_range) | Temp_range==""))
d13nAS4PT_genus <- amp_subset_samples(d13nAS4PT_genus, !(is.na(ClimateZone2) | ClimateZone2==""))
d13nAS4PT_genus <- amp_subset_samples(d13nAS4PT_genus, !(is.na(Continent) | Continent==""))
d13nAS4PT_genus <- amp_subset_samples(d13nAS4PT_genus, !(is.na(ind_text) | ind_text==""))
d13nAS4PT_genus <- amp_subset_samples(d13nAS4PT_genus, !(is.na(Process_type) | Process_type==""))

# Order factors
d13nAS4PT_genus$metadata$Temp_range <- factor(d13nAS4PT_genus$metadata$Temp_range, levels=c("very low", "low", "moderate", "high", "very high", "extremely high"))
d13nAS4PT_genus$metadata$ind_text <- factor(d13nAS4PT_genus$metadata$ind_text, levels=c("none", "very_low", "low", "medium", "high", "all"))
d13nAS4PT_genus$metadata$Process_type <- factor(d13nAS4PT_genus$metadata$Process_type, levels=c("C", "C,N", "C,N,DN", "C,N,DN,P"))
...
p1a <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="none",
             distmeasure = "bray",
...
p1b <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="pa",
             distmeasure = "bray",
             sample_color_by = "Process_type",
...
# RDA plot
p1c_raw <- amp_ordinate(d13nAS4PT_genus,
             type = "RDA",
...
# PCoA plot (Bray-Curtis diversity of genera)
p2a <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="none",
             distmeasure = "bray",
...
# PCoA plot (Soerensen diversity of genera)
p2b <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="pa",
             distmeasure = "bray",
             sample_color_by = "ind_text",
...
# RDA plot
p2c_raw <- amp_ordinate(d13nAS4PT_genus,
             type = "RDA",
             constrain = "ind_text",
             sample_color_by = "ind_text",
             sample_point_size = 3,
...
######################
# PCoA plot (Bray-Curtis diversity of genera)
p3a <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="none",
             distmeasure = "bray",
...
# PCoA plot (Soerensen diversity of genera)
p3b <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="pa",
             distmeasure = "bray",
             sample_color_by = "Temp_range",
...
# RDA plot
p3c_raw <- amp_ordinate(d13nAS4PT_genus,
             type = "RDA",
...
######################
# PCoA plot (Bray-Curtis diversity of genera)
p4a <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="none",
             distmeasure = "bray",
...
# PCoA plot (Soerensen diversity of genera)
p4b <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="pa",
             distmeasure = "bray",
             sample_color_by = "Continent",
...
# RDA plot
p4c_raw <- amp_ordinate(d13nAS4PT_genus,
             type = "RDA",
             constrain = "Continent",
             sample_color_by = "Continent",
             sample_point_size = 3,
...
######################
# PCoA plot (Bray-Curtis diversity of genera)
p5a <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="none",
             distmeasure = "bray",
...
# PCoA plot (Soerensen diversity of genera)
p5b <- amp_ordinate(d13nAS4PT_genus,
             type = "PCoA",
             transform="pa",
             distmeasure = "bray",
             sample_color_by = "ClimateZone2",
...
# RDA plot
p5c_raw <- amp_ordinate(d13nAS4PT_genus,
             type = "RDA",
...
ggsave(filename="output/Figure5_V13_Beta-diversity_PCoA.pdf", plot=p1, width=22, height=10, useDingbats=FALSE, limitsize=FALSE)

p2 <- ( p1c | p2c | p2c )  / (p3c | p4c | p5c )
ggsave(filename="output/FigureS5_V13_Beta-diversity_RDA.pdf", plot=p2, width=15, height=11.5, useDingbats=FALSE, limitsize=FALSE)
...
write.csv(RDA_scores,"output/DataS3_V13.csv", row.names = FALSE, quote = FALSE)

######################
#~ADONIS analyses 
######################

bc.dist.matrix <- vegdist(t(d13nAS4PT_genus$abund), method = "bray")
s.dist.matrix <- vegdist(t(d13nAS4PT_genus$abund), method = "bray", binary = TRUE)
metadata <- d13nAS4PT_genus$metadata
